  ...
  if (page === 'center') {
    ...
    function renderCenter() {
      api('/api/state').then(function (s) {
        ...
        $('#c-msgs').innerHTML = agBanner + (msgs.length ? msgs.map(function (m) { return '<div class="review" style="' + (m.unread ? 'border-left:3px solid #e5484d;padding-left:12px' : '') + '"><div class="review-head"><span class="u">' + esc(m.from) + (m.unread ? ' <span style="color:#e5484d;font-size:11px">未读</span>' : '') + '</span><span class="date">' + new Date(m.t).toLocaleString('zh-CN') + '</span></div><div style="font-weight:600;margin-bottom:4px">' + esc(m.subject) + '</div><div class="text">' + m.body + '</div></div>'; }).join('') : '<div style="padding:30px;text-align:center;color:#9aa1ab">暂无站内信。</div>');
